<template>
    ...
    >
      ...
      <div class="flex items-center gap-[10px] cursor-pointer" @click="scrollToReviews" v-if="reviewGetters.getTotalReviews(reviewAverage) > 0">        
        <div class="flex flex-col w-[calc(12*5px+4px)] xl:w-[calc(15*5px+4px)]">
          <div class="inline-flex gap-[1px] h-[12px] xl:h-[15px]">
            <svg v-for="i in 5" :key="i" class="w-[12px] h-[12px] xl:w-[15px] xl:h-[15px] fill-[#999] flex-shrink-0" viewBox="0 0 15 15"><use href="#svg_star"></use></svg>
          </div>
          <div class="inline-flex gap-[1px] overflow-hidden h-[12px] xl:h-[15px] mt-[-12px] xl:mt-[-15px]" :style="'width:' + reviewGetters.getAverageRating(reviewAverage, 'half') * 20 + '%'">
            <svg v-for="i in 5" :key="i" class="w-[12px] h-[12px] xl:w-[15px] xl:h-[15px] fill-[#FCC72F] flex-shrink-0" viewBox="0 0 15 15"><use href="#svg_star"></use></svg>
          </div>
        </div>

        <div class="text-[12px] underline xl:text-[14px]">{{ reviewGetters.getTotalReviews(reviewAverage) }} {{ reviewGetters.getTotalReviews(reviewAverage) === 1 ? t('Product.review') : t('Product.reviews') }}</div>
      ...
      <div v-if="!showPayPalButtons" class="pt-[20px] flex flex-col gap-[10px]">
      ...
      <ul class="pt-[20px] text-[12px] xl:text-[14px]">
        <li><strong>{{ t('content') }}:</strong> {{ product.unit.content }} {{ product.unit.names.name }}</li>
        <li><strong>{{ t('Product.itemNo') }}:</strong> {{ product.variation.model }}</li>
      </ul>
    </form>
  </template>
...
// oekotex badge
const oekotex = computed(() => {
  const vp = props?.product?.variationProperties;
  if (!vp) return false;

  // Find group with id 4 (Zusätliche Artikel-Informationen)
  const group = vp.find(v => v.id === 4);
  if (!group || !Array.isArray(group.properties)) return false;

  // Check if it contains property id 46 (Oeko Tex Siegel auf Produktseite anzeigen)
  return group.properties.some(p => p.id === 46);
});
